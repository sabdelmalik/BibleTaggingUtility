...
{
    internal class TranslatorTagsEx
    {
        ...
        public TranslatorTagsEx()
        {
            ...
            //                              ch  v  s  l  v    s   l
            otVerseMap[vRef].Add(new int[] { ch, v, 0, 10, v, 0, 11 });
            ...
            otVerseMap[vRef].Add(new int[] { ch, v, 13, 0, v, 14, 33 });
            ...
            otVerseMap[vRef].Add(new int[] { ch, v, 5, 0, v, 5, 18 });
            ...
            otVerseMap[vRef].Add(new int[] { ch, v, 5, 0, v, 6, 31 });
            ...
            otVerseMap[vRef].Add(new int[] { ch, v, 12, 0, v, 14, 25 });
                        ...
                        foreach (StrongsNumber strong in verse[i].Strong.Strongs)
                        {
                            ...
                            if (hwList.Count > 1)
                            {
                                ...
                                else
                                {
                                    ...
                                    List<VerseWord> unused = new List<VerseWord>();
                                    bool found = false;
                                    ...
                                    if (unused.Count > 0)
                                    {
                                        ...
                                        if (unused.Count > 1)
                                        {
                                            ...
                                            for (int c = 1; c < unused.Count; c++)
                                            {
                                                int sp = Math.Abs(lastHebIndex - unused[c].WordIndex);
                                                if (sp < span)
                                                {
                                                    span = sp;
                                                    hw = unused[c];
...
*/
                ...
                // fill in the missing Arabic Words
                // Arabic words with no hw, are assigned hebrew word numbers beyond the hebrewVerse.Count
                int arabicWordCount = end - startIndex;
                int missingArabicWords = arabicWordCount - map.Count;
                ...
                // make room at the end for the unmapped Arabic words
                ...
                {
                    if((aIndex % constructedAllawance) == 0)
                    {
                        int adjIndex = (aIndex / constructedAllawance) - startIndex - 1;
                        if (hIndex.Count > 0)
        ...
        private void OutputTranslatorTagsOT(string destination, Dictionary<string, List<TranslatorWord>> words, bool publicDomain)
        {
            ...
            using (StreamWriter swR = new StreamWriter(requireReview, false))
            {
